#devtools::install() # install wham and whamMSE from the github repo (https://github.com/lichengxue/MSE_NAA_Project)
library(wham)
library(whamMSE)
...
info <- generate_basic_info(n_stocks = 2,
                            ...
                            Fbar_ages = 12,
                            recruit_model = 2,
                            F_info = list(F.year1 = 0.2, Fhist = "F-H-L", Fmax = 2.5, Fmin = 1, change_time = 0.5),
...
# Check brp
exp(om$rep$log_FMSY_static)
exp(om$rep$log_FXSPR_static)
exp(om$rep$log_FMSY_static) - exp(om$rep$log_FXSPR_static)
