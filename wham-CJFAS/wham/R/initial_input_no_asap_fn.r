...
initial_input_no_asap_fn <- function(input, basic_info){
  ...
  if(!is.null(basic_info$fracyr_seasons)) {
    ...
    if(length(basic_info$fracyr_seasons) != 1) input$log$misc <- c(input$log$misc, "basic_info$fracyr_seasons implies more than one season. Ensure that input$data$fracyr_SSB, input$data$spawn_seasons, input$data$fracyr_indices, input$data$index_seasons, are specified apropriately. \n ")
  }
